...
// 1.ngx_accept_disabled 是否可以接收新的连接
// 2.ngx_trylock_accept_mutex抢锁，抢到锁的人才能accept
// 3.ngx_process_events 处理网络IO相关的事件(accept/connect/read/write)
// 4.ngx_event_process_posted 处理accept_posted_events，为了快速释放锁，accept事件会优先处理，其他事件会稍候处理
...
// 处理socket读写事件和定时器事件
// 获取负载均衡锁，监听端口接受连接
// 调用epoll模块的 ngx_epoll_process_events 获取发生的事件
...
ngx_process_events_and_timers(ngx_cycle_t *cycle)
{
    ...
    if (ngx_use_accept_mutex) {
        ...
        if (ngx_accept_disabled > 0) {
            ...
            // ngx_trylock_accept_mutex 执行成功
            // 使用变量 ngx_accept_mutex_held 检查是否成功获取了锁

            // 确实已经获得了锁，接下来的 epoll 的事件需要加入延后队列处理
            // 这样可以尽快释放锁给其他进程，提高运行效率
            if (ngx_accept_mutex_held) {

                // 加上NGX_POST_EVENTS标志
                // epoll获得的所有事件都会加入到 ngx_posted_events
                ...
                // 也就是说nginx的epoll不会等待超过 ngx_accept_mutex_delay 的500毫秒
    ...
    // 实际上就是 ngx_epoll_process_events
    //
    // epoll模块核心功能，调用epoll_wait处理发生的事件
    // 使用event_list 和 nevents 获取内核返回的事件
    // timer 是无事件发生时最多等待的时间，即超时时间
    // 如果 ngx_event_find_timer 返回timer ==0，那么epoll不会等待，立即返回
...
//event模块初始化, 主要功能是创建共享内存和accept互斥锁
static ngx_int_t
...
//https://blog.csdn.net/cywosp/article/details/7032371
//初始化事件驱动器，连接池，定时器，以及挂在listen 句柄的回调函数。
static ngx_int_t
ngx_event_process_init(ngx_cycle_t *cycle)
{
    ...
    //master进程打开，worker进程数大于1，配置了accetp_mutex(默认使用)时，才使用accept互斥体。
...
#endif
        /*
         * 没有使用accept互斥体，那么就在此处将监听套接字放入 epoll中。
         * ngx_event_actions
        */
        if (ngx_add_event(rev, NGX_READ_EVENT, 0) == NGX_ERROR) {
            return NGX_ERROR;
...
ngx_event_use(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (m = 0; cf->cycle->modules[m]; m++) {
        ...
        if (module->name->len == value[1].len) {
            //验证event模块名称是否一致，一致的话则设置其标号 ecf->use
