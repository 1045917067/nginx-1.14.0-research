...
所需标志位：
END_STREAM (0x1): 报头块为最后一个，意味着流的结束。后续可紧接着CONTINUATION帧在当前的流中，
                  需要把CONTINUATION帧作为HEADERS帧的一部分对待
END_HEADERS (0x4): 此报头帧不需分片，完整的一个帧。后续不再需要CONTINUATION帧帮忙凑齐。
                   若没有此标志的HEADER帧，后续帧必须是以CONTINUATION帧传递在当前的流中，
                   否则接收者需要响应PROTOCOL_ERROR类型的连接错误。
PADDED (0x8): 需要填充的标志
...
/*
 setting帧决定接收到HEAD帧后创建流的时候，确定发送窗口的大小和流窗口大小。同时决定在通过write chain发送frame帧的时候，
 决定没个数据块的大小
 例如一个frame帧为10K，但是对端指定其接收数据的frame_size=5K，则该frame会被拆成两个frame_size大小的包体发送
 数据帧发送优先级控制见ngx_http_v2_queue_frame
*/
...
// flag带有该标识，表示内容部分带有Stream Dependency和weight
...
typedef struct {
    ...
    ngx_http_v2_header_t             header;  // 赋值 见ngx_http_v2_get_indexed_header
...
struct ngx_http_v2_connection_s {
    ...
    /*
     ...
     默认NGX_HTTP_V2_DEFAULT_WINDOW 65535
    */
    size_t                           send_window;
    ...
    /*
     ...
     默认NGX_HTTP_V2_MAX_WINDOW 2^32 -1
    */
    size_t                           recv_window;
    ...
    /*
     ...
     流的发送窗口可以通过setting帧调整，但是接受窗口都是默认值NGX_HTTP_V2_MAX_WINDOW
     默认NGX_HTTP_V2_DEFAULT_WINDOW  决定流的发送窗口大小 一般对端会发送SETTING帧进行调整
    */
    size_t                           init_window;

    /*
     接收到对端的setting帧后，会做调整，见ngx_http_v2_state_settings_params 决定发送FRAME帧的时候，每帧数据大小不超过该值
     默认NGX_HTTP_V2_DEFAULT_FRAME_SIZE  接收到对端的setting帧后，会做调整，见ngx_http_v2_state_settings_params
    */
    size_t                           frame_size;
    ...
    /*
     ...
     真正的ngx_http_v2_node_t赋值见ngx_http_v2_get_node_by_id
    */
...
/*
 ...
 接收到header帧的时候，在ngx_http_v2_get_node_by_id
*/
...
struct ngx_http_v2_stream_s {
    ...
    */
    // 默认等于h2c->init_window，也就是65535
    ssize_t                          send_window;
    // 默认值NGX_HTTP_V2_MAX_WINDOW 2^32 - 1
    size_t                           recv_window;
...
/*
 创建空间和赋值见ngx_http_v2_send_settings，对应一种setting、HEADER等帧，每个帧对应一个该结构，
 最终加入ngx_http_v2_connection_t->last_out
...
struct ngx_http_v2_out_frame_s {
    ...
    ngx_chain_t                     *last;
    /*
     赋值:ngx_http_v2_settings_frame_handler
